...
export function NewWorkOrderModal({ isOpen, onClose, onSuccess, formData }: NewWorkOrderModalProps) {
  ...
  const [pesoPerColloEffettivo, setPesoPerColloEffettivo] = useState('');
  ...
  const validate = (): boolean => {
    ...
    const parsedPeso = Number(pesoPerColloEffettivo);
    if (!pesoPerColloEffettivo || Number.isNaN(parsedPeso) || parsedPeso <= 0) {
      nextErrors.pesoPerColloEffettivo = 'Inserisci un peso per collo maggiore di zero.';
    }
    ...
    try {
      ...
      const lavorazione = await createLavorazione(
        supabase,
        {
          ...
          pesoPerColloEffettivo: Number(pesoPerColloEffettivo),
          note: note.trim() || null,
          apertaAt,
                ...
                <label className="block text-sm font-medium text-slate-700">
                  ...
                  <Select
                    ...
                    onChange={(event) => {
                      ...
                      if (nextArticolo) {
                        setPesoPerColloEffettivo(String(nextArticolo.pesoPerCollo));
                        resetFieldError('pesoPerColloEffettivo');
                      }
                    }}
                  >
                ...
                <label className="block text-sm font-medium text-slate-700">
                  ...
                  <div className="flex items-center gap-2">
                    <Input
                      ...
                      step="0.001"
                      min="0.001"
                      value={pesoPerColloEffettivo}
                      onChange={(event) => {
                        setPesoPerColloEffettivo(event.target.value);
                        resetFieldError('pesoPerColloEffettivo');
                      }}
                    />
                    {selectedArticolo && Number(pesoPerColloEffettivo) !== selectedArticolo.pesoPerCollo ? (
                  ...
                  {errors.pesoPerColloEffettivo ? (
                    <p className="mt-1 text-xs text-red-500">{errors.pesoPerColloEffettivo}</p>
                  ) : null}
                </label>
