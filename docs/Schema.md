...
**Contatore giornaliero atomico + trigger generazione `codice_pedana`:**
```sql
-- Contatori giornalieri per progressivo pedane (evita race condition)
CREATE TABLE contatori_giornalieri (
  giorno date PRIMARY KEY,
  conteggio_pedane integer NOT NULL DEFAULT 0
);
...
BEGIN
  ...
  INSERT INTO contatori_giornalieri (giorno, conteggio_pedane)
  VALUES (oggi, 1)
  ON CONFLICT (giorno) DO UPDATE
  SET conteggio_pedane = contatori_giornalieri.conteggio_pedane + 1
  RETURNING conteggio_pedane INTO progressivo;
...
Questa strategia usa un incremento atomico per giorno e garantisce un progressivo univoco anche con inserimenti concorrenti.
